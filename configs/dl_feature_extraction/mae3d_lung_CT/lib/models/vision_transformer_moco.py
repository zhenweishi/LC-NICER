...
__all__ = [
    ...
    "vit_3d_base_patchsize8_stemconv"
]
...
def vit_3d_base_patchsize8_stemconv(**kwargs):
    model = VisionTransformerMoCo3D(
        patch_size=8, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), embed_layer=PatchEmbed3D_Deform, **kwargs)
    model.default_cfg = _cfg()
    return model